...
int input_row(matrix_t *matrix, const int row, FILE *file)
{
...
int input_matrix(matrix_t *matrix)
{
    ...
    for (int i = 0; i < matrix->rows; i++)
        if (input_row(matrix, i, stdin) != EXIT_SUCCESS)
        {
            free_matrix(matrix);
            return INPUT_ERR;
        }
...
int read_matrix_from_file(matrix_t *matrix, const char *filename)
{
    ...
    for (int i = 0; i < matrix->rows; i++)
        if (input_row(matrix, i, file) != EXIT_SUCCESS)
        {
            free_matrix(matrix);
            return INPUT_ERR;
        }
